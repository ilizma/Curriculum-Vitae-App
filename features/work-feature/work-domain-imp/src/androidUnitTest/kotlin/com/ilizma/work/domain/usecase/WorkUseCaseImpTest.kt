package com.ilizma.work.domain.usecase

import com.ilizma.curriculum.domain.model.CurriculumVitae
import com.ilizma.curriculum.domain.repository.CurriculumVitaeRepository
import com.ilizma.work.domain.mapper.WorkMapper
import com.ilizma.work.domain.model.Work
import io.mockk.MockKAnnotations
import io.mockk.coEvery
import io.mockk.every
import io.mockk.impl.annotations.RelaxedMockK
import kotlinx.coroutines.test.runTest
import kotlin.test.BeforeTest
import kotlin.test.Test
import kotlin.test.assertEquals
import com.ilizma.curriculum.domain.model.Work as CVWork

class WorkUseCaseImpTest {

    @RelaxedMockK
    private lateinit var repository: CurriculumVitaeRepository

    @RelaxedMockK
    private lateinit var mapper: WorkMapper

    @RelaxedMockK
    private lateinit var curriculumVitae: CurriculumVitae

    @RelaxedMockK
    private lateinit var cvWork: CVWork

    @RelaxedMockK
    private lateinit var work: Work

    private lateinit var useCase: WorkUseCase

    init {
        MockKAnnotations.init(this)

        every { curriculumVitae.work } returns listOf(cvWork)
        coEvery { repository.get() } returns curriculumVitae
        every { mapper.from(listOf(cvWork)) } returns listOf(work)
    }

    @BeforeTest
    fun setup() {
        useCase = WorkUseCaseImp(
            repository = repository,
            mapper = mapper,
        )
    }

    @Test
    fun `given curriculumVitae, when invoked, then result should be the expected workList`() =
        runTest {
            // given
            val expected = listOf(work)

            // when
            val result = useCase()

            // then
            assertEquals(expected, result)
        }
    
}